...
open class GalleryViewController: UIPageViewController, ItemControllerDelegate {
    ...
    fileprivate func layoutScrubber() {
        ...
        scrubber.bounds = CGRect(origin: CGPoint.zero, size: CGSize(width: self.view.bounds.width, height: 100))
        scrubber.center = self.view.boundsCenter
        scrubber.frame.origin.y = (footerView?.frame.origin.y ?? self.view.bounds.maxY) - (scrubber.bounds.height - 50)
    }
